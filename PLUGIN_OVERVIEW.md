...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb canvas` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb canvas list`, add
one todo at a time with `bb canvas add`, and close finished work with
`bb canvas done`.
