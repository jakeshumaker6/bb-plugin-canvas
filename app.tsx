...
const PLACEMENT_OFFSETS: Record<Exclude<BoardNodeKind, "image">, { x: number; y: number }> = {
  sticky: { x: 110, y: 80 },
...
function ToolButton({ item, active, onClick }: { item: (typeof DOCK_ITEMS)[number]; active: boolean; onClick: () => void }) {
  return (
    <button
  ...
  return (
    <div className="selection-toolbar" role="toolbar" aria-label="Selection properties" style={position} onPointerDown={(event) => event.stopPropagation()}>
      ...
      {typographic ? <>
        ...
        <button className={`selection-action text-action ${lead.fontWeight >= 700 ? "is-active" : ""}`} aria-label="Bold" title="Bold" onClick={() => onApply({ fontWeight: lead.fontWeight >= 700 ? 500 : 700 })}>B</button>
      ...
      <button className="selection-action" aria-label={`Duplicate selected ${noun}`} title="Duplicate (Cmd/Ctrl+D)" onClick={onDuplicate}><Icon name="Copy" className="size-4" /></button>
      <button
...
function ConnectorToolbar({
  ...
  return (
    <div className="selection-toolbar connector-toolbar" role="toolbar" aria-label="Connector properties" style={position} onPointerDown={(event) => event.stopPropagation()}>
      <input
...
function BoardWorkspace({ board, onBoardChange }: { board: BoardDocument; onBoardChange: (board: BoardDocument) => void }) {
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "Enter" && chosen.length === 1 && selectedNow.length === 1) {
        event.preventDefault();
      ...
      if (event.key === "Escape") { setSelectedIds([]); setEditingId(null); setSelectedEdgeId(null); setConnectorSource(null); setShapeMenuOpen(false); setColorMenuOpen(false); setInspectorOpen(false); setTool("select"); }
    };
  ...
  const createNode = useCallback((kind: BoardNodeKind, x: number, y: number) => {
    if (kind === "image") return;
    const id = globalThis.crypto.randomUUID();
    const offset = PLACEMENT_OFFSETS[kind];
    void persist([{ type: "add_node", node: { id, kind, x: x - offset.x, y: y - offset.y, color: kind === "sticky" ? "yellow" : kind === "text" ? "white" : "blue" } }]);
    setSelectedIds([id]);
    setEditingId(id);
    setSelectedEdgeId(null);
    ...
    <div className="canvas-stage">
      ...
      >
        <div className="canvas-world" style={{ transform: `translate(${viewport.x}px, ${viewport.y}px) scale(${viewport.zoom})` }}>
          <svg className="canvas-edges" width="10000" height="10000" aria-label="Connectors">
            <defs><marker id="canvas-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker></defs>
            {board.edges.map((edge) => {
              ...
              return (
                <g key={edge.id}>
                  ...
                  <path
                    className={`canvas-edge ${edge.id === selectedEdgeId ? "is-selected" : ""}`}
                    d={path}
                    fill="none"
