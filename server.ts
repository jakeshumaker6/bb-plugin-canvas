...
// surfaces: the Example todos page (app.tsx, over RPC), the `bb canvas` CLI
...
export default async function plugin(bb: BbPluginApi) {
  ...
  // `bb plugin config canvas`. Add `secret: true` for values like API keys.
  ...
  // The `bb canvas` command: what agents (and you) use from a shell. Parsing
  ...
  const usage = [
    "Usage:",
    "  bb canvas list [--json]",
    "  bb canvas add <title> [--json]",
    "  bb canvas done <todo-id> [--json]",
    "  bb canvas undo <todo-id> [--json]",
    "  bb canvas remove <todo-id> [--json]",
  ].join("\n");
  ...
  bb.cli.register({
    name: "canvas",
    summary: "Manage the Canvas plugin's example todo list",
    commands: [
      { name: "list", summary: "List todos", usage: "bb canvas list [--json]" },
      {
        name: "add",
        summary: "Add a todo",
        usage: "bb canvas add <title> [--json]",
      },
      {
        name: "done",
        summary: "Mark a todo done",
        usage: "bb canvas done <todo-id> [--json]",
      },
      {
        name: "undo",
        summary: "Mark a todo not done",
        usage: "bb canvas undo <todo-id> [--json]",
      },
      {
        name: "remove",
        summary: "Remove a todo",
        usage: "bb canvas remove <todo-id> [--json]",
      },
    ],
    async run(argv) {
      ...
      const notFound = (missingId: string) => ({
        exitCode: 1,
        stderr: `No todo with id ${missingId}. Run "bb canvas list" to see ids.`,
      });
      const todoId = args[0];
      switch (command) {
        case undefined:
